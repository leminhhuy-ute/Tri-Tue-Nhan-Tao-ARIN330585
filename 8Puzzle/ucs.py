"""
8 Puzzle - Uniform Cost Search (UCS)
Giải bài toán 8-Puzzle bằng thuật toán tìm kiếm chi phí đồng nhất.
Sử dụng hàng đợi ưu tiên (heapq), mỗi bước di chuyển có chi phí = 1.
File standalone - chạy trực tiếp: python ucs.py
"""
...
def solve_ucs(start):
    ...
    Mỗi bước có chi phí = 1. Dùng hàng đợi ưu tiên (min-heap).
    Đảm bảo tìm được đường đi có chi phí nhỏ nhất.
    ...
    trace = []
    # Heap: (chi_phí, bộ_đếm, trạng_thái, đường_đi)
    counter = 0  # Dùng để phá vỡ tie khi chi phí bằng nhau
    heap = [(0, counter, start, [start])]
    best_cost = {start: 0}  # Chi phí tốt nhất để đến mỗi trạng thái
    explored = 0
    last_logged_cost = -1
    ...
    while heap:
        ...
        # Log theo chi phí
        if cost > last_logged_cost:
            last_logged_cost = cost
            trace.append(
                f"── Chi phí {cost} | Heap: {len(heap)} | Đã duyệt: {explored}"
            )

        if state == GOAL:
            trace.append(f"\n✔ Tìm thấy lời giải với chi phí {cost}!")
            trace.append(f"  Tổng nút duyệt: {explored}")
            trace.append(f"  Độ dài đường đi: {cost} bước")
            return path, explored, trace

        for move_name, neighbor in get_neighbors(state):
            new_cost = cost + 1  # Mỗi bước chi phí = 1
...
class PuzzleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("8 Puzzle - UCS (Uniform Cost Search)")
    ...
    def _build_gui(self):
        ...
        tk.Label(
            title_frame, text="8 PUZZLE — UCS", font=("Arial", 18, "bold"),
            fg=HIGHLIGHT, bg=BG
        ).pack(side=tk.LEFT)

        tk.Label(
            title_frame, text="Uniform Cost Search (Tìm kiếm chi phí đồng nhất)",
        ...
        self.status_var = tk.StringVar(value="Sẵn sàng | Thuật toán: UCS (Uniform Cost Search)")
        tk.Label(
    ...
    def _solve(self):
        ...
        self._append_trace("  THUẬT TOÁN: UCS (Uniform Cost Search)")
        self._append_trace("  Tìm kiếm chi phí đồng nhất")
        self._append_trace("=" * 45)
        self._append_trace(f"\nTrạng thái đầu:\n{board_text(state)}")
        self._append_trace(f"\nManhattan distance ban đầu: {manhattan(state)}")
        self._append_trace(f"Chi phí mỗi bước: 1")
        self._append_trace("\nĐang giải...\n")

        self.root.update()

        t0 = time.perf_counter()
        path, explored, trace_lines = solve_ucs(state)
        ...
        self._append_trace(f"  Tổng chi phí đường đi: {total}")
        ...
        for i in range(1, len(path)):
            ...
            self._append_trace(f"Bước {i} (chi phí: {i}): Di chuyển ô {moved_val}")

        self._update_nav()
        self.status_var.set(
            f"Đã giải xong | Bước: {total} | Chi phí: {total} | Nút duyệt: {explored} | "
    ...
    def _reset(self):
        ...
        self.status_var.set("Sẵn sàng | Thuật toán: UCS (Uniform Cost Search)")
    ...
    def _update_nav(self):
        ...
        if self.step_index == 0:
            ...
            blank_curr = curr.index(0)
            moved_val = prev[blank_curr]
            self.info_label.config(text=f"Di chuyển ô {moved_val} (chi phí: {self.step_index})")
