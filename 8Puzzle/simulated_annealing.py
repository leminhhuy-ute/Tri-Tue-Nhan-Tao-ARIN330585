...
Nhiệt độ bắt đầu T=1000, tốc độ làm nguội 0.995.
Chấp nhận bước đi tệ hơn với xác suất e^(delta_h / T).
Hiển thị nhiệt độ và xác suất chấp nhận trong trace.
"""
...
def simulated_annealing(start):
    """
    Simulated Annealing: chọn láng giềng ngẫu nhiên.
    Nếu tốt hơn → chấp nhận. Nếu tệ hơn → chấp nhận với xác suất e^(Δh/T).
    """
    current = start
    h = manhattan(current)
    path = [current]
    trace = [f"Bắt đầu: h={h}, T={T_INIT}"]
    ...
    for step in range(MAX_STEPS):
        if current == GOAL:
            trace.append(f"✅ Đạt trạng thái đích tại bước {step}! T={T:.2f}")
            return path, trace

        if T < 1e-10:
            trace.append(f"⛔ Nhiệt độ quá thấp (T≈0). Dừng tại h={h}.")
            return path, trace

        neighbors = get_neighbors(current)
        move, ns, nh = random.choice(neighbors)
        delta = h - nh  # delta > 0 nghĩa là ns tốt hơn

        if delta > 0:
            # Láng giềng tốt hơn → chấp nhận
            trace.append(
                f"Bước {step+1}: '{move}' h={h}→{nh} (tốt hơn) | T={T:.2f}"
            )
            current = ns
            h = nh
            path.append(current)
        else:
            # Láng giềng tệ hơn → chấp nhận với xác suất
            prob = math.exp(delta / T)
            r = random.random()
            if r < prob:
                trace.append(
                    f"Bước {step+1}: '{move}' h={h}→{nh} (tệ hơn, "
                    f"p={prob:.4f} > r={r:.4f} → CHẤP NHẬN) | T={T:.2f}"
                )
                current = ns
                h = nh
                path.append(current)
                ...
                trace.append(
                    f"Bước {step+1}: '{move}' h={h}→{nh} (tệ hơn, "
                    f"p={prob:.4f} ≤ r={r:.4f} → TỪ CHỐI) | T={T:.2f}"
                )
    ...
    if current == GOAL:
        trace.append(f"✅ Đạt đích!")
    else:
        trace.append(
            f"⛔ Đạt giới hạn {MAX_STEPS} bước. h hiện tại={h}, best h={best_h}, T={T:.4f}"
        )
    return path, trace
...
class PuzzleApp:
    ...
    def _build_ui(self):
        tk.Label(
            self.root, text="8 Puzzle – Simulated Annealing",
        ...
        # Tham số SA
        param_frame = tk.Frame(left, bg=PANEL)
        param_frame.pack(pady=(0, 8))
        tk.Label(param_frame, text=f"T₀={T_INIT}  α={COOLING_RATE}",
                 font=("Consolas", 9), fg=HIGHLIGHT, bg=PANEL).pack()
    ...
    def _solve(self):
        ...
        if start is None:
            return
        self.path, self.trace_lines = simulated_annealing(start)
        self.step_idx = 0
        ...
        res = "Thành công ✅" if reached else "Chưa đạt đích ⛔"
