...
Giải bài toán 8-Puzzle bằng thuật toán tìm kiếm theo chiều rộng.
Sử dụng hàng đợi (deque) để duyệt theo từng mức.
File standalone - chạy trực tiếp: python bfs.py
"""
...
def solve_bfs(start):
    ...
    Duyệt lần lượt từng mức, đảm bảo tìm được đường đi ngắn nhất.
    ...
    trace = []
    queue = deque()
    queue.append((start, [start]))  # (trạng_thái, đường_đi)
    visited = {start}
    explored = 0
    current_depth = 0
    ...
    while queue:
        ...
        # Log khi chuyển sang mức mới
        if depth > current_depth:
            current_depth = depth
            trace.append(f"── Mức {current_depth} | Frontier: {len(queue)} | Đã duyệt: {explored}")

        if state == GOAL:
            trace.append(f"✔ Tìm thấy lời giải tại mức {depth}!")
...
class PuzzleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("8 Puzzle - BFS (Breadth-First Search)")
    ...
    def _build_gui(self):
        ...
        tk.Label(
            title_frame, text="8 PUZZLE — BFS", font=("Arial", 18, "bold"),
            fg=HIGHLIGHT, bg=BG
        ).pack(side=tk.LEFT)

        tk.Label(
            title_frame, text="Breadth-First Search (Tìm kiếm theo chiều rộng)",
        ...
        self.status_var = tk.StringVar(value="Sẵn sàng | Thuật toán: BFS (Breadth-First Search)")
        tk.Label(
    ...
    def _solve(self):
        ...
        self._append_trace("  THUẬT TOÁN: BFS (Breadth-First Search)")
        self._append_trace("  Tìm kiếm theo chiều rộng")
        self._append_trace("=" * 45)
        ...
        path, explored, trace_lines = solve_bfs(state)
        ...
        if path is None:
            ...
            self.status_var.set(f"Không có lời giải | Đã duyệt: {explored} nút | Thời gian: {self.solve_time:.3f}s")
            return
    ...
    def _reset(self):
        ...
        self.status_var.set("Sẵn sàng | Thuật toán: BFS (Breadth-First Search)")
