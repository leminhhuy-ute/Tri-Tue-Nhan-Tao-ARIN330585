"""
8 Puzzle - Stochastic Hill Climbing
Chọn NGẪU NHIÊN một láng giềng có h thấp hơn trạng thái hiện tại.
Dừng nếu không có láng giềng nào tốt hơn.
"""
...
def stochastic_hill_climbing(start):
    """
    Stochastic Hill Climbing: chọn ngẫu nhiên một láng giềng có h nhỏ hơn.
    """
    current = start
    h = manhattan(current)
    path = [current]
    ...
    for step in range(MAX_STEPS):
        ...
        # Lọc các láng giềng tốt hơn
        better = [(m, s, nh) for m, s, nh in neighbors if nh < h]

        if not better:
            trace.append(
                f"⛔ Kẹt tại cực tiểu địa phương (h={h}), "
                f"không có láng giềng tốt hơn trong {len(neighbors)} láng giềng."
            )
            return path, trace

        # Chọn ngẫu nhiên từ các láng giềng tốt hơn
        move, ns, nh = random.choice(better)
        trace.append(
            f"Bước {step+1}: Chọn ngẫu nhiên '{move}' → h={nh} "
            f"(từ {len(better)} láng giềng tốt hơn, giảm từ {h})"
        )
        current = ns
        h = nh
        path.append(current)
...
class PuzzleApp:
    ...
    def _build_ui(self):
        tk.Label(
            self.root, text="8 Puzzle – Stochastic Hill Climbing",
    ...
    def _solve(self):
        ...
        if start is None:
            return
        self.path, self.trace_lines = stochastic_hill_climbing(start)
        self.step_idx = 0
