# =============================================================================
# 8 Puzzle - A* Search
# Tìm kiếm A* với f(n) = g(n) + h(n), h = Manhattan Distance
# File standalone - chạy trực tiếp: python astar.py
# =============================================================================
...
def astar_search(start):
    ...
    h(n) = manhattan distance (heuristic chấp nhận được)
    ...
    open_heap = [(f_start, counter, g_start, start, [start])]
    # best_g[state] = chi phí tốt nhất đã biết để đến state
    best_g = {start: 0}
    trace = []
    nodes_explored = 0

    trace.append(f"═══ A* SEARCH ═══")
    trace.append(f"f(n) = g(n) + h(n)")
    trace.append(f"Trạng thái đầu: {start}")
    trace.append(f"g(start) = {g_start}, h(start) = {h_start}, f(start) = {f_start}")
    trace.append(f"{'─' * 45}")

    while open_heap:
        f, _, g, current, path = heapq.heappop(open_heap)

        # Nếu đã tìm thấy đường đi tốt hơn đến current, bỏ qua
        ...
        trace.append(f"[Bước {nodes_explored}] Mở rộng nút | g={g}, h={h}, f={f}")
        trace.append(board_text(current))
        trace.append("")

        if current == GOAL:
            trace.append(f"✓ ĐÃ TÌM THẤY ĐÍCH!")
            trace.append(f"Số bước (tối ưu): {len(path) - 1}")
        ...
        for move_name, neighbor in get_neighbors(current):
            ...
            if new_g < best_g.get(neighbor, float('inf')):
                ...
                f_n = new_g + h_n
                counter += 1
                heapq.heappush(open_heap, (f_n, counter, new_g, neighbor, path + [neighbor]))
                trace.append(f"  → {move_name}: g={new_g}, h={h_n}, f={f_n}")
...
class PuzzleApp:
    ...
    def _build_ui(self):
        ...
        tk.Label(
            title_frame, text="🧩 8 Puzzle — A* Search",
            font=("Arial", 16, "bold"), fg=self.HIGHLIGHT, bg=self.BG
        ).pack(side=tk.LEFT)

        tk.Label(
            title_frame, text="f(n) = g(n) + h(n) | Optimal & Complete",
    ...
    def _solve(self):
        ...
        t0 = time.time()
        path, nodes, trace = astar_search(self.start_state)
        t1 = time.time()
