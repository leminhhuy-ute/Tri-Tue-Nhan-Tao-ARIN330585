"""
IDS (Iterative Deepening Search) - Máy Hút Bụi
...
def ids_search(start_state):
    ...
    for limit in range(26):  # depth limit from 0 to 25
        stack = [(start_state, [], 0)]
        ...
        while stack:
            ...
            if is_goal(current):
                frontier_names = [get_name(s) for s, _, _ in stack]
                all_trace.append({
                    "step": global_step,
                    "node": current_name,
                    "frontier": f"GOAL (Limit={limit})",
            ...
            children_added = []
            if depth < limit:
                # Add neighbors in reverse order so they are popped correctly in DFS
                for action, next_state in reversed(get_neighbors(current)):
                    if next_state not in discovered:
                        discovered.add(next_state)
                        child_name = get_name(next_state)
                        stack.append((next_state, path + [(action, next_state)], depth + 1))
                        children_added.append(child_name)
            
            frontier_items = [get_name(s) for s, _, _ in stack]
            frontier_text = f"[Limit={limit}] " + ("[" + ", ".join(frontier_items) + "]" if frontier_items else "[]")
            if children_added:
                frontier_text = "Thêm: " + ", ".join(reversed(children_added)) + "\n" + frontier_text
            elif depth >= limit:
                frontier_text = "Cắt tỉa (đạt limit)\n" + frontier_text
...
class VacuumApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Máy Hút Bụi - IDS (Iterative Deepening Search)")
    ...
    def _build_gui(self):
        title = tk.Label(self.root, text="MÁY HÚT BỤI — IDS (Iterative Deepening Search)",
    ...
    def solve(self):
        try:
            ...
            self.solution, self.trace, self.node_names = ids_search(self.start_state)
            self.current_step = 0
            self.draw_grid(self.start_state)
            self.show_trace()

            if self.solution is None:
                self.status_label.config(text="IDS: Không tìm thấy lời giải (vượt quá depth limit)!")
            ...
            self.status_label.config(text=f"IDS: Tìm thấy lời giải gồm {len(self.solution)} bước!")
