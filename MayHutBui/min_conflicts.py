...
def run_algorithm(start_state, app):
    app.append_trace("Min-Conflicts: Chọn bước đi giảm thiểu xung đột (Heuristic)...")
    path = []
    current = start_state
    
    def h(st):
        d = [(r, c) for r in range(len(st[2])) for c in range(len(st[2][0])) if st[2][r][c] == 1]
        return len(d) * 10 + (min(abs(st[0]-dr)+abs(st[1]-dc) for dr,dc in d) if d else 0)

    visited_counts = {}
    
    for step in range(150): # Cho phép tối đa 150 bước
        if is_goal(current):
            app.append_trace("Đã dọn sạch mọi xung đột (Rác)!")
            break
            
        visited_counts[current] = visited_counts.get(current, 0) + 1
        neighbors = get_neighbors(current)
        if not neighbors: 
            app.append_trace("Bị kẹt không có đường đi!")
            break
            
        # Đánh giá xung đột: heuristic + phạt nếu đã thăm nhiều lần
        best_acts = []
        min_conflict = float('inf')
        
        for act, st in neighbors:
            conflict = h(st) + visited_counts.get(st, 0) * 8
            if conflict < min_conflict:
                min_conflict = conflict
                best_acts = [(act, st)]
            elif conflict == min_conflict:
                best_acts.append((act, st))
                
        # Chọn ngẫu nhiên trong số các lựa chọn tốt nhất
        act, st = random.choice(best_acts)
        
        app.append_trace(f"Xung đột ước tính = {min_conflict}. Đi {act}.")
        path.append((st, f"Min-Conflicts: Đi {act}"))
        current = st
        
    if not is_goal(current):
        app.append_trace("Không thể dọn sạch sau 150 bước (Đạt giới hạn).")
...
class VacuumApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Máy Hút Bụi - Min-Conflicts")
    ...
    def _build_gui(self):
        tk.Label(self.root, text="MÁY HÚT BỤI — Min-Conflicts", font=("Segoe UI", 16, "bold"), bg="#2d3436", fg="white").pack(pady=(10, 5))
