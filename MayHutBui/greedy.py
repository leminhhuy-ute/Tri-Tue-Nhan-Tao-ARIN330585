"""
Greedy Best-First Search - Máy Hút Bụi
Thuật toán tìm kiếm tốt nhất đầu tiên (h(n)).
...
def greedy_search(start_state):
    ...
    while pq:
        ...
        if is_goal(current):
            frontier_names = [f"{get_name(s)}(h={h})" for h, _, s, _ in pq]
            trace.append({
                "step": step,
                "node": f"{current_name}(h={h_val})",
                "frontier": "GOAL",
        ...
        for action, next_state in get_neighbors(current):
            if next_state not in discovered:
                ...
                heapq.heappush(pq, (h_next, tie_breaker, next_state, path + [(action, next_state)]))
                tie_breaker += 1
                children_added.append(f"{child_name}(h={h_next})")
                
        frontier_items = [f"{get_name(s)}(h={h})" for h, _, s, _ in sorted(pq)]
...
class VacuumApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Máy Hút Bụi - Greedy Best-First Search")
    ...
    def _build_gui(self):
        title = tk.Label(self.root, text="MÁY HÚT BỤI — Greedy Best-First Search",
        ...
        self.algo_var = tk.StringVar(value="Greedy Search")
        algo_menu = ttk.Combobox(control_panel, textvariable=self.algo_var, state="disabled",
                                 values=["Greedy Search"], width=30, font=("Segoe UI", 10))
    ...
    def make_trace_table(self):
        step_w, node_w, frontier_w, reached_w = 6, 12, 45, 25
    ...
    def solve(self):
        try:
            ...
            self.solution, self.trace, self.node_names = greedy_search(self.start_state)
            self.current_step = 0
            self.draw_grid(self.start_state)
            self.show_trace()

            if self.solution is None:
                self.status_label.config(text="Greedy: Không tìm thấy lời giải!")
            ...
            self.status_label.config(text=f"Greedy: Tìm thấy lời giải gồm {len(self.solution)} bước!")
