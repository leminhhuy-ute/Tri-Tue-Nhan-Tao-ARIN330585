"""
A* Search - Máy Hút Bụi
Thuật toán A* (f(n) = g(n) + h(n)).
...
def astar_search(start_state):
    ...
    pq = []
    # (f(n), tie_breaker, g(n), state, path)
    h_start = heuristic(start_state)
    heapq.heappush(pq, (h_start, 0, 0, start_state, []))
    ...
    while pq:
        ...
        # Bỏ qua nếu đã tìm được đường tốt hơn đến trạng thái này
        ...
        if is_goal(current):
            frontier_names = [f"{get_name(s)}(f={f})" for f, _, g, s, _ in pq if g <= best_g.get(s, float('inf'))]
            trace.append({
                "step": step,
        ...
        for action, next_state in get_neighbors(current):
            ...
            if new_g < best_g.get(next_state, float('inf')):
                ...
                f_next = new_g + h_next
                heapq.heappush(pq, (f_next, tie_breaker, new_g, next_state, path + [(action, next_state)]))
                tie_breaker += 1
                children_added.append(f"{child_name}(f={f_next})")
                
        # Lọc frontier để hiển thị
        valid_pq = [(f, s) for f, _, g, s, _ in pq if g <= best_g.get(s, float('inf'))]
        frontier_items = [f"{get_name(s)}(f={f})" for f, s in sorted(valid_pq)]
...
class VacuumApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Máy Hút Bụi - A* Search")
    ...
    def _build_gui(self):
        title = tk.Label(self.root, text="MÁY HÚT BỤI — A* Search",
        ...
        algo_menu = ttk.Combobox(control_panel, textvariable=self.algo_var, state="disabled",
                                 values=["A* Search"], width=30, font=("Segoe UI", 10))
    ...
    def solve(self):
        try:
            ...
            self.solution, self.trace, self.node_names = astar_search(self.start_state)
            self.current_step = 0
            self.draw_grid(self.start_state)
            self.show_trace()

            if self.solution is None:
                self.status_label.config(text="A*: Không tìm thấy lời giải!")
            ...
            self.status_label.config(text=f"A*: Tìm thấy lời giải gồm {len(self.solution)} bước!")
