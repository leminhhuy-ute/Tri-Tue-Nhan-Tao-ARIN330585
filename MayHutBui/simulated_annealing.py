"""
Simulated Annealing - Máy Hút Bụi
Mô phỏng luyện kim.
...
def simulated_annealing(start_state):
    ...
    trace.append(f"KHỞI ĐẦU: h={current_h}, T={T}")
    ...
    while step < 1000:
        ...
        if is_goal(current):
            trace.append(f"\nBước {step}: ĐÃ TÌM THẤY LỜI GIẢI! (T={T:.2f})")
            return path, trace
            
        if T <= 0.0001:
            trace.append(f"\nBước {step}: Nhiệt độ quá thấp (T={T:.5f}), thuật toán dừng.")
            break
        ...
        delta_E = current_h - next_h # E = h, ta muốn h giảm (nhỏ hơn tốt hơn). Nếu delta_E > 0 thì tốt
        
        if delta_E > 0:
            # Láng giềng tốt hơn -> chấp nhận
            trace.append(f"Bước {step}: Tốt hơn (h={next_h} < {current_h}) -> CHẤP NHẬN '{action}'. T={T:.2f}")
            current = next_state
            current_h = next_h
            path.append((action, next_state))
        else:
            # Láng giềng tệ hơn -> xác suất chấp nhận
            prob = math.exp(delta_E / T)
            if random.random() < prob:
                trace.append(f"Bước {step}: Tệ hơn (h={next_h} >= {current_h}), xác suất {prob:.3f} -> CHẤP NHẬN '{action}'. T={T:.2f}")
                current = next_state
                current_h = next_h
                path.append((action, next_state))
            else:
                trace.append(f"Bước {step}: Tệ hơn (h={next_h} >= {current_h}), xác suất {prob:.3f} -> TỪ CHỐI. T={T:.2f}")
                
        T = T * cooling_rate
        
    trace.append("\nDừng thuật toán do chưa đạt goal.")
    return None, trace
...
class VacuumApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Máy Hút Bụi - Simulated Annealing")
    ...
    def _build_gui(self):
        title = tk.Label(self.root, text="MÁY HÚT BỤI — Simulated Annealing",
        ...
        algo_menu = ttk.Combobox(control_panel, textvariable=self.algo_var, state="disabled",
                                 values=["Simulated Annealing"], width=25, font=("Segoe UI", 10))
    ...
    def solve(self):
        try:
            ...
            self.solution, self.trace = simulated_annealing(self.start_state)
            self.current_step = 0
            self.draw_grid(self.start_state)
            self.show_trace()

            if self.solution is None:
                self.status_label.config(text="Simulated Annealing: Thất bại!")
                return

            self.status_label.config(text=f"Simulated Annealing: Tìm thấy lời giải sau {len(self.solution)} bước!")
