"""
IDA* (Iterative Deepening A*) - Máy Hút Bụi
Thuật toán tìm kiếm sâu dần với ngưỡng f(n).
...
def ida_star_search(start_state):
    ...
    while bound < 1000: # safety limit
        stack = [(start_state, [], 0)]
        path_set = {start_state}
        discovered_in_iter = {start_state}
        next_bound = float('inf')
        found = False
        solution = None
        
        while stack:
            current, path, g_val = stack.pop()
            global_step += 1
            current_name = get_name(current)
            h_val = heuristic(current)
            f_val = g_val + h_val
            
            # Xóa khỏi path_set khi quay lui
            if len(path) > 0 and path[-1][1] in path_set and current != path[-1][1]:
                # Cần xử lý cẩn thận path_set trong stack-based DFS
                # Ở bản đơn giản này, ta cho phép thăm lại các trạng thái ở nhánh khác
                pass
            
            if f_val > bound:
                next_bound = min(next_bound, f_val)
                frontier_items = [f"{get_name(s)}(f={g+heuristic(s)})" for s, p, g in stack]
                frontier_text = f"[B={bound}] " + ("[" + ", ".join(frontier_items) + "]" if frontier_items else "[]")
                frontier_text = f"Cắt tỉa (f={f_val} > {bound})\n" + frontier_text
                
                reached_list = [node_names[s] for s in list(node_names.keys()) if s in discovered_in_iter]
                all_trace.append({
                    "step": global_step,
                    "node": f"{current_name}(f={f_val})",
            ...
            if is_goal(current):
                frontier_names = [f"{get_name(s)}(f={g+heuristic(s)})" for s, p, g in stack]
                all_trace.append({
                    "step": global_step,
                    "node": f"{current_name}(f={f_val})",
                    "frontier": f"GOAL (Bound={bound})",
                    "reached": ", ".join(node_names[s] for s in list(node_names.keys()) if s in discovered_in_iter)
                })
                return path, all_trace, node_names
                
            children_added = []
            for action, next_state in reversed(get_neighbors(current)):
                # Tránh chu trình trong đường đi hiện tại
                in_path = False
                for p_act, p_st in path:
                    if p_st == next_state:
                        in_path = True
                        break
                ...
                if not in_path:
                    discovered_in_iter.add(next_state)
                    child_name = get_name(next_state)
                    stack.append((next_state, path + [(action, next_state)], g_val + 1))
                    f_next = g_val + 1 + heuristic(next_state)
                    children_added.append(f"{child_name}(f={f_next})")
                    
            frontier_items = [f"{get_name(s)}(f={g+heuristic(s)})" for s, p, g in stack]
            frontier_text = f"[B={bound}] " + ("[" + ", ".join(frontier_items) + "]" if frontier_items else "[]")
            if children_added:
                frontier_text = "Thêm: " + ", ".join(reversed(children_added)) + "\n" + frontier_text
    ...
    return None, all_trace, node_names
...
class VacuumApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Máy Hút Bụi - IDA* Search")
    ...
    def _build_gui(self):
        title = tk.Label(self.root, text="MÁY HÚT BỤI — IDA* Search",
        ...
        self.algo_var = tk.StringVar(value="IDA* Search")
        algo_menu = ttk.Combobox(control_panel, textvariable=self.algo_var, state="disabled",
                                 values=["IDA* Search"], width=30, font=("Segoe UI", 10))
    ...
    def make_trace_table(self):
        step_w, node_w, frontier_w, reached_w = 6, 15, 45, 22
    ...
    def solve(self):
        try:
            ...
            self.solution, self.trace, self.node_names = ida_star_search(self.start_state)
            self.current_step = 0
            self.draw_grid(self.start_state)
            self.show_trace()

            if self.solution is None:
                self.status_label.config(text="IDA*: Không tìm thấy lời giải!")
            ...
            self.status_label.config(text=f"IDA*: Tìm thấy lời giải gồm {len(self.solution)} bước!")
