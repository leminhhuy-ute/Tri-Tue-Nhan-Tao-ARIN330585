...
def run_algorithm(start_state, app):
    app.append_trace("CSP Backtracking: Khởi chạy tìm kiếm đệ quy (Recursive DFS)...")
    
    def backtrack(current_state, path, visited):
        if is_goal(current_state):
            return path
        if len(path) > 20:
            return None
            
        for act, next_st in get_neighbors(current_state):
            if next_st not in visited:
                app.append_trace(f"Thử {act} tại bước {len(path)+1}")
                res = backtrack(next_st, path + [(next_st, f"Backtrack: Đi {act}")], visited | {next_st})
                if res:
                    return res
                app.append_trace(f"Quay lui (Backtrack) từ {act}")
        return None

    res = backtrack(start_state, [], {start_state})
    if res:
        app.append_trace("Đã tìm thấy lịch trình dọn dẹp hoàn chỉnh!")
        return res
    else:
        app.append_trace("Không tìm được đường đi hoàn chỉnh trong giới hạn 20 bước.")
        return []
...
class VacuumApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Máy Hút Bụi - CSP Backtracking")
    ...
    def _build_gui(self):
        tk.Label(self.root, text="MÁY HÚT BỤI — CSP Backtracking", font=("Segoe UI", 16, "bold"), bg="#2d3436", fg="white").pack(pady=(10, 5))
