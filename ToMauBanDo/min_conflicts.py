"""
Tô Màu Bản Đồ - Min-Conflicts
Sử dụng tìm kiếm cục bộ để giải CSP.
"""
...
class MinConflictsApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Tô Màu Bản Đồ - Min-Conflicts")
    ...
    def _build_ui(self):
        ...
        tk.Label(title_frame, text="🗺️ Tô Màu Bản Đồ - Min-Conflicts",
    ...
    def _draw_graph(self):
        ...
        self.canvas.create_text(280, 25, text="Min-Conflicts Local Search", font=("Segoe UI", 13, "bold"), fill="#2c3e50")
    ...
    def _get_conflicts(self, assign):
        conflicts = 0
        conflicted_vars = set()
        for r1, r2 in ADJACENCY:
            if assign[r1] == assign[r2]:
                conflicts += 1
                conflicted_vars.add(r1)
                conflicted_vars.add(r2)
        return conflicts, list(conflicted_vars)

    def _generate_steps(self):
        self.steps = []
        assign = {r: random.choice(COLOR_NAMES) for r in REGIONS}
        
        self.steps.append({"assign": dict(assign), "msg": "Khởi tạo màu ngẫu nhiên."})
        
        for i in range(100):
            c_count, c_vars = self._get_conflicts(assign)
            if c_count == 0:
                self.steps.append({"assign": dict(assign), "msg": f"Đã giải quyết sau {i} bước!"})
                return
            
            var = random.choice(c_vars)
            
            best_color = assign[var]
            min_c = 999
            for color in COLOR_NAMES:
                assign[var] = color
                temp_c, _ = self._get_conflicts(assign)
                if temp_c < min_c:
                    min_c = temp_c
                    best_color = color
                    
            assign[var] = best_color
            self.steps.append({"assign": dict(assign), "msg": f"Bước {i+1}: Chọn {var} = {best_color} để giảm xung đột."})

        self.steps.append({"assign": dict(assign), "msg": "Không tìm thấy lời giải sau 100 bước."})
